...
REST API for TupiaManager - a pricing engine and administrative system that calculates furniture quotes based on customer-supplied dimensions and a bill of materials defined per product.
...
[controllers/config/multer.ts](controllers/config/multer.ts) configures Multer with `CloudinaryStorage`. Uploaded files are stored in Cloudinary folder `tupia-manager-produtos`. The `file.path` returned by Cloudinary is the public URL stored in `product.photos`.
