...
test.describe("CRUD /api/products (protegido) + /api/products/quote/:id", () => {
  ...
  test.beforeAll(async () => {
    ...
    if (!matRes.ok())
      throw new Error(
        `beforeAll: falha ao criar material (${matRes.status()})`,
      );
    ...
    if (!prodRes.ok())
      throw new Error(
        `beforeAll: falha ao criar produto (${prodRes.status()}): ${await prodRes.text()}`,
      );
  ...
  test("PATCH /:id 200 atualização parcial", async ({
    request,
    authHeaders,
  }) => {
    const res = await request.patch(`/api/products/${productId}`, {
  ...
  test("POST /quote/:id 200 dimensões válidas → retorna preço", async ({
    request,
  }) => {
    const res = await request.post(`/api/products/quote/${productId}`, {
  ...
  test("POST /quote/:id 400 profundidade fora dos limites", async ({
    request,
  }) => {
    const res = await request.post(`/api/products/quote/${productId}`, {
  ...
  test("POST /quote/:id 404 produto inexistente", async ({ request }) => {
    const res = await request.post(
      "/api/products/quote/000000000000000000000000",
      {
        data: DIMENSIONS_VALID,
      },
    );
  ...
  // --- limitar profitMargin entre 0 e 300% ---
  test("deve retornar 400 quando profitMargin acima de 300", async () => {
    const { token } = JSON.parse(fs.readFileSync(STATE_FILE, "utf-8"));
    const ctx = await playwrightRequest.newContext({
      baseURL: BASE_URL,
      extraHTTPHeaders: { Authorization: `Bearer ${token}` },
    ...
    const res = await ctx.post("/api/products", {
      data: {
        name: "Armario Margem Invalida",
        description: "Produto para testar teto de profitMargin",
        constraints: CONSTRAINTS,
        components: [
          {
            material: materialId,
